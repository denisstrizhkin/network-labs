...
Для оценки производительности производилось измерение **коэффициента эффективности** ($\eta$), который рассчитывается как:
...
1. При нулевых потерях оба протокола имеют $\eta = 1$.
...
3. Selective Repeat демонстрирует линейное снижение эффективности, пропорциональное уровню потерь ($\eta \approx 1 - P_{loss}$), так как пересылаются только потерянные пакеты. При высоких потерях (>60%) преимущество SR становится подавляющим.
